...
@PutMapping("/{enterpriseId}/session-feedback-form")
    fun saveForm(
        ...
        if (request.headerText.isBlank())
            errors.add("headerText is required and cannot be blank")
        if (request.headerText.length > 100)
            errors.add("headerText must not exceed 100 characters")
        ...
        if (request.ratingLabels.size != 5)
            errors.add("ratingLabels must contain exactly 5 items")
        if (request.ratingLabels.any { it.isBlank() })
            errors.add("ratingLabels must not contain blank values")
        if (request.thankYouText.isBlank())
            errors.add("thankYouText is required and cannot be blank")
        if (request.thankYouText.length > 255)
            errors.add("thankYouText must not exceed 255 characters")
        if (request.invalidReplyText.isBlank())
            errors.add("invalidReplyText is required and cannot be blank")
        if (request.expiredReplyText.isBlank())
            errors.add("expiredReplyText is required and cannot be blank")
        ...
        if (invalidChannels.isNotEmpty())
            errors.add("Invalid channels: $invalidChannels. Allowed: $allowedChannels")
